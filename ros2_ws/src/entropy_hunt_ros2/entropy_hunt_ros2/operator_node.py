"""
Entropy Hunt ROS2 Operator Node

Listens to simulated PX4-compatible swarm telemetry, aggregates state,
and exposes snapshot and health endpoints for ground-station integration.
"""
...
if Node is not None:
    ...
    class _OperatorNodeImpl(Node):
        def __init__(self) -> None:
            ...
            self.state = OperatorState.create(
                drone_count=self.params.drone_count,
                grid_size=self.params.grid,
            )
            ...
            self._mesh_publishers = {
                "runtime_control": self.create_publisher(
                    BINDINGS.runtime_control.message_type,
                    BINDINGS.runtime_control.name,
                    BINDINGS.runtime_control.qos,
                ),
            }
            self._subs = [
                ...
                create_subscription(self, BINDINGS.consensus_result, self._on_consensus_result),
            ...
            self.server: OperatorSnapshotServer | None = None
            if self.params.snapshot_path:
                self.server = OperatorSnapshotServer(
        ...
        def _on_heartbeat(self, message: Any) -> None:
            try:
                ...
                self.get_logger().error(f"_on_heartbeat error: {e}")
                return
        ...
        def _on_drone_state(self, message: Any) -> None:
            try:
                apply_drone_state(
                    ...
                    altitude_m=float(getattr(message, "altitude_m", 0.0)),
                    heading_deg=float(getattr(message, "heading_deg", 0.0)),
                    battery_pct=float(getattr(message, "battery_pct", 100.0)),
                    flight_mode=str(getattr(message, "flight_mode", "UNKNOWN")),
                )
            except Exception as e:
                self.get_logger().error(f"_on_drone_state error: {e}")
                return
        ...
        def _on_certainty_update(self, message: Any) -> None:
            try:
                ...
                self.get_logger().error(f"_on_certainty_update error: {e}")
                return
        ...
        def _build_snapshot(self) -> dict[str, Any]:
            ...
            snapshot["config"]["snapshot_url"] = (
                f"http://{self.params.snapshot_host}:{self.params.snapshot_port}/snapshot.json"
            )
            ...
            snapshot["config"]["control_capabilities"] = self._build_control_payload()[
        ...
        def _health_supplier(self) -> dict[str, Any]:
            peers_online = sum(
                1 for d in self.state.drones.values()
                if d.alive and d.reachable
            )
            return {"status": "ok", "peers_online": peers_online}
        ...
        def _apply_control_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
            ...
            if isinstance(next_tick, (int, float)) and float(next_tick) > 0:
                ...
                if normalized_tick != self._control_tick_seconds:
                    ...
                    changed = True
            if (
                isinstance(next_count, int)
                and next_count > 0
                and next_count != self._requested_drone_count
            ):
            ...
            if changed:
                ...
                self.get_logger().info(
                    f"Operator control updated: "
                    f"tick={self._control_tick_seconds}s "
        ...
        def _report(self) -> None:
            try:
                ...
                self.get_logger().info(
                    f"Operator snapshot: "
                    f"drones={len(self.state.drones)}/{self.params.drone_count} "
                    f"coverage={snapshot['stats']['coverage']} "
                    f"survivor={snapshot['survivor_found']}",
                )
                write_snapshot_file(self.params.snapshot_path, snapshot)
            except Exception as e:
                self.get_logger().error(f"_report error: {e}")
                return
else:

    class _OperatorNodePlaceholder:
        pass
...
def main(args: list[str] | None = None) -> int:
    ...
    try:
        ...
        if hasattr(node, "server") and node.server is not None:
            node.server.stop()
        node.destroy_node()
        rclpy.shutdown()
    return 0
