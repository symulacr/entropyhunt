...
- **Task 4**: Additional work (1): READ FULLY: entropy_hunt.md + entropy_hunt_mockup.html + en
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   `omx team api send-message --input "{"team_name":"read-fully-entropy-hunt-md-ent","from_worker":"worker-4","to_worker":"leader-fixed","body":"ACK: worker-4 initialized"}" --json`
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/home/kpa/entropyhunt/.omx/state/team/read-fully-entropy-hunt-md-ent/workers/worker-4/status.json`
...
1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"read-fully-entropy-hunt-md-ent","worker":"worker-4"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"read-fully-entropy-hunt-md-ent","worker":"worker-4","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"read-fully-entropy-hunt-md-ent","from_worker":"worker-4","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
